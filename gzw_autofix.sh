...
_read_game_state() {
    ...
    # FIX #4: Beide Teile müssen non-empty sein
...
# NOTE: Both .clean_checksums and .last_known_state reside in the game's cache directory.
# A full reinstall (not "Verify integrity") will delete them, forcing a new baseline on next launch.
...
# FIX #4: Leerstring abfangen (buildid oder depot_manifests fehlte)
...
for FILE in "${FILES[@]}"; do
    ...
    if [ ! -f "$FULL_PATH" ]; then
        ...
        # FIX #2: Fehlende Spieldatei ist ein Fehler, kein Skip
    ...
    if [ ! -f "$BACKUP_PATH" ] || [ "$UPDATE_DETECTED" = true ]; then
        ...
        # FIX #3: Backup-Pfad wird korrekt als Skip gezählt (kein Restore nötig/möglich)
    ...
    # FIX #1: Arithmetik-Increment nicht via ((...)) — exit code 1 bei Ergebnis 0
